...
/**
 * Replace all the characters into upper case.
 * @author fredy
 */
public class AllUpperCaseReplacerImpl extends AbstractReplacer {

    @Override
    public List<String> replace(List<String> fileNames) {
        List<String> newFileNames = new ArrayList<String>();
        for (String fileName : fileNames) {
            newFileNames.add(toUpperCase(fileName));
        }
        return newFileNames;
    }

    private String toUpperCase(String fileName) {
        String fullPath = FileNameUtils.getFullPath(fileName);
        String srcFileName = FileNameUtils.getBaseName(fileName);
        String extension = FileNameUtils.getExtension(fileName);
        String destFileName = fullPath + srcFileName.toUpperCase() + extension;
